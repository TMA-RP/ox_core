...
export class OxPlayer extends ClassInterface {
	...
	async setActiveCharacter(data: number | NewCharacter) {
		...
		const sendToClient = ['blood', 'inDuty', 'inProperty', 'inGarage', 'hasCompletedBoostingTabletStuff']
		const setAsStatebag = ['inDuty', 'underPiquouze', 'autoHeal', 'headbag', 'instance', 'deathTimestamp']
		// Values stored in metadata and synced to client.
		if (character.data) {
			const cData = JSON.parse(character.data)
			for (const key in cData) {
				this.set(key, cData[key], sendToClient.includes(key));
				if (setAsStatebag.includes(key)) state.set(key, cData[key], true);
			}
		}
